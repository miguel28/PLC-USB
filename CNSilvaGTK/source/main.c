...
static void draw_circle(GtkWidget *widget, gdouble basex, gdouble basey, gint radio,int size){
	gdk_draw_arc(pixMap,widget->style->black_gc,FALSE,basex-radio,basey-radio,radio*2,radio*2,0,360*64);
...
static gboolean button_press_event( GtkWidget *widget, GdkEventButton *event )
{
	if (event->button == 1 && pixMap != NULL){
		if (Clicked==FALSE){
			...
			cx1=(event->x/ZOOM)-OffsetPiece;
			cy1=((event->y/ZOOM)-OffsetPiece-PartHeight)*-1;
			rx1=(cx1+OffsetPiece)*ZOOM;
			ry1=((cy1*-1)+OffsetPiece+PartHeight)*ZOOM;
			Clicked=TRUE;
			draw_circle(widget, rx1, ry1, 6,6);
		}else{
			cx2=(event->x/ZOOM)-OffsetPiece;
			cy2=((event->y/ZOOM)-OffsetPiece- PartHeight)*-1;
			...
			Points[PointCounter].x=cx1; Points[PointCounter].y=cy1;
			PointCounter++;
			Points[PointCounter].x=cx2; Points[PointCounter].y=cy2;
			PointCounter++;
...
static gboolean motion_notify_event( GtkWidget *widget, GdkEventMotion *event )
{
	...
	if(Clicked==TRUE){
		Render(widget);
		draw_line (widget, rx1, ry1,event->x, event->y,LINEWIDTH);
